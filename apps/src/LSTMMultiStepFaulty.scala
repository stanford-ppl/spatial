...
object LSTMMultiStepFaulty extends SpatialApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    type FixAccum = FixPt[TRUE, _16, _16]
    ...
    val ijfoDRAMs: scala.List[DRAM2[T]] = scala.List.tabulate(nGates) { _ =>
      val re: DRAM2[T] = DRAM[T](cHiddenUnits, cFeatures + cHiddenUnits)
      re
    }

    val ijfoData: scala.List[Matrix[T]] = List.tabulate(nGates) { num =>
    ...
    def getVecDRAMNum(num: I32, size: I32): DRAM1[T] = {
      val re = DRAM[T](size)
      val reData = Array.tabulate[T](size)(i => ((i + num).to[T] * 0.25))
    ...
    val xhData = (0 :: cTimeSteps + 1, 0 :: cFeatures + cHiddenUnits) {
      (r, c) =>
    ...
    val biasesDRAM: List[DRAM1[T]] = List.tabulate(nGates) { n =>
    ...
    Accel {
      ...
      //load weight
      val ijfoMems: List[SRAM2[T]] = List.tabulate(nGates) { _ =>
        val re: SRAM2[T] =
      ...
      //load bias
      val biasesMems: List[SRAM1[T]] = List.tabulate(nGates) { _ =>
          ...
          accumRegs.zip(ijfoMems).foreach {
            case (acc, w) =>
              val t = List
                ...
                .sumTree
              val tmpTVal: FixAccum = t.to[FixAccum]
              val accVal: FixAccum = acc.value.to[FixAccum]
              val accUpdate = mux(iuvTile == 0.to[I32], tmpTVal, tmpTVal + accVal)
              acc := accUpdate.to[T]
          }
      ...
      cResultDRAM(0.to[I32] :: nHiddenUnits par storePar) store c(
        0.to[I32] :: nHiddenUnits)
      hResultDRAM(0.to[I32] :: nTimeSteps,
                  0.to[I32] :: nHiddenUnits par storePar) store xh(
        1.to[I32] :: nTimeSteps + 1,
        nFeatures :: (nFeatures + nHiddenUnits))
    }
    ...
    val tanhHost: Array[T] => Array[T] = x => {
      Array.tabulate[T](x.length)(i => tanh(x(i)))
      //x
    }
    def sigHost: Array[T] => Array[T] = x => {
      Array.tabulate[T](x.length)(i => sigmoid(x(i)))
      //x
    }
    ...
    def variance(a: Tensor1[T], b: Tensor1[T]): T =
      Array
        .tabulate[T](nHiddenUnits) { ih =>
          pow(abs(a(ih) - b(ih)), 2)
        }
        .reduce(_ + _) / 128
