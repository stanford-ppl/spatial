...
import argon.node.GenericToText
...
abstract class Top[A](implicit ev: A <:< Ref[Any,A]) extends Ref[Any,A] { self =>
  ...
  @api def neql(that: A): Bit = Bit(!this.equals(that))
  @api def eql(that: A): Bit = Bit(this.equals(that))
