...
trait Num[A] extends Order[A] with Arith[A] with Bits[A] {
  ...
  @api def ::(start: A): Series[A]  = Series[A](start, me, one, I32(1), isUnit = false)
  @api def par(p: I32): Series[A]   = Series[A](zero, me, one, p, isUnit = false)
  @api def by(step: A): Series[A]   = Series[A](zero, me, step, I32(1), isUnit = false)
  @api def until(end: A): Series[A] = Series[A](me, end, one, I32(1), isUnit = false)

  @api def toSeries: Series[A] = Series[A](me, me+one, one, I32(1), isUnit=true)
