...
trait Typing { this: Printing =>

  implicit class InvertOps[A](x: A) {
    def view[B[_]<:Type[_]](implicit tp: B[A]): B[A] = x match {
      ...
      case _ => throw new Exception(c"Cannot view $x as a $tp ($x is a ${x.getClass}, not a symbol)")
    }
    def viewSym(implicit tp: Type[A]): Sym[A] = x match {
      ...
      case _ => throw new Exception(c"Cannot view $x as a Sym[$tp] ($x is a ${x.getClass}, not a symbol)")
    }
