...
trait IR {
  implicit def castTypedAsSym[A:Type](x: A): Sym[A] = typ[A].viewAsSym(x)

  implicit class TypedUtils[A:Type](x: A) {
    def asTop: Top[A] = x.asInstanceOf[Top[A]]
  }
