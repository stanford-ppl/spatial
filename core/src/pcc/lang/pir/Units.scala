...
case class PMUPtr(protected[pir] var unit: Option[VPMU] = None) extends PUPtr {
  ...
  @api def read(r0: Series)(path: I32 => I32): Void = this.read(Seq(r0)){i => path(i(0)) }
  @api def read(r0: Series, r1: Series)(path: (I32,I32) => I32): Void = this.read(Seq(r0,r1)){i => path(i(0),i(1)) }
  @api def read(r0: Series, r1: Series, r2: Series)(path: (I32,I32,I32) => I32): Void = this.read(Seq(r0,r1,r2)){i => path(i(0),i(1),i(2)) }

  @api def read(ranges: Seq[Series])(path: Seq[I32] => I32): Void = {
    ...
    curPtr.withPtr(this){
      ...
      val block = stageBlock{
        val ctrs = ranges.map{r => Counter.fromSeries(r) }
        CounterChain(ctrs:_*)
  ...
  @api def write(r0: Series)(path: I32 => (I32,Bits[_])): Void = this.write(Seq(r0)){i => path(i(0)) }
  @api def write(r0: Series, r1: Series)(path: (I32,I32) => (I32,Bits[_])): Void = this.write(Seq(r0,r1)){i => path(i(0),i(1)) }
  @api def write(r0: Series, r1: Series, r2: Series)(path: (I32,I32,I32) => (I32,Bits[_])): Void = this.write(Seq(r0,r1,r2)){i => path(i(0),i(1),i(2)) }

  @api def write(ranges: Seq[Series])(path: Seq[I32] => (I32,Bits[_])): Void = {
    val u = unit.getOrElse{throw new Exception("Modified uninitialized PMUPtr") }
    curPtr.withPtr(this){
      val indices = ranges.map{_ => bound[I32] }
      val block = stageBlock{
...
case class CurrentPtr(ptr: PUPtr) extends SimpleData[CurrentPtr]
@data object curPtr {
...
object PCU {
  ...
  @api def apply(r0: Series)(datapath: I32 => Void): PCUPtr = PCU.apply(Seq(r0)){is => datapath(is.head)}
  @api def apply(r0: Series, r1: Series)(datapath: (I32,I32) => Void): PCUPtr = PCU.apply(Seq(r0,r1)){is => datapath(is(0),is(1)) }
  @api def apply(r0: Series, r1: Series, r2: Series)(datapath: (I32,I32,I32) => Void): PCUPtr = PCU.apply(Seq(r0,r1,r2)){is => datapath(is(0),is(1),is(2)) }

  @api def apply(ranges: Seq[Series])(datapath: Seq[I32] => Void): PCUPtr = {
    val ptr = PCUPtr()
    curPtr.withPtr(ptr) {
      val indices = ranges.map{_ => bound[I32] }
      val block = stageBlock {
