...
class FIFOArbiter[T<:Data] (val t: T, val d: Int, val v: Int, val numStreams: Int) extends Module {
  ...
  if (numStreams > 0) {
    ...
    // RR Arb using 2 Priority  encoder and request mask
    val reqMasks = Fill(numStreams,true.B) << (tag + 1.U);

    val activeFifo = PriorityEncoder(fifoValids)
    val fifoValidsMasked =  Vec(List.tabulate(numStreams) { i => fifoValids(i) & reqMasks(i) })
    val activeFifo_mask = PriorityEncoder(fifoValidsMasked)
    tagFF.io.in := Mux(fifoValidsMasked.reduce{_|_}, activeFifo_mask , activeFifo)
