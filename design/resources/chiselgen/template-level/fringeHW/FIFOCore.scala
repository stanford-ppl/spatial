...
class FIFOCore[T<:Data](override val t: T, override val d: Int, override val v: Int, override val banked: Boolean=false, val useFF: Boolean=false) extends FIFOBase(t, d, v, banked) {
  ...
  val mems = List.fill(bankCount) {
    List.fill(v) {
      if (w == 1 || banked || depth == 1 || useFF) Module(new FFRAM(t, depth)) else {
        val sram = Module(new SRAM(t, depth, "VIVADO_SELECT"))
        sram.io.flow := true.B
        sram.io.raddrEn := true.B
        sram
      }
    }
  }
