...
class SRAM(val logicalDims: List[Int], val bitWidth: Int, 
  ...
  io.output.data.zip(bankIdR).foreach { case (wire, id) => 
    val id_retimed = Utils.getRetimed(id, Utils.sramload_latency)
    val sel = (0 until numMems).map{ i => (id_retimed === i.U)}
