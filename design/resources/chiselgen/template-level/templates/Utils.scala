...
object VCU1525 extends DeviceTarget
    ...
    def *-* (c: SInt, delay: Option[Double]): SInt = { // TODO: Find better way to capture UInt / UInt, since implicit resolves won't make it this far
      if (Utils.retime) {
        ...
        Utils.target match {
          ...
          case VCU1525 => b.asSInt*c // Raghu's box
    ...
    def *-* (c: UInt, delay: Option[Double]): SInt = { // TODO: Find better way to capture UInt / UInt, since implicit resolves won't make it this far
      if (Utils.retime) {
        ...
        Utils.target match {
          ...
          case VCU1525 => b*c.asSInt // Raghu's box
...
object Utils {
  ...
  def fix2flt(num: FixedPoint, m: Int, e: Int): FloatingPoint = {
    val s = num.s
    val d = num.d
    val f = num.f
    val result   = Wire(new FloatingPoint(m, e))
    result.r := FringeGlobals.bigIP.fix2flt(num.r, s, d, f, m, e) 
    result
  }

  def flt2fix(num: FloatingPoint, s: Boolean, d: Int, f: Int): FixedPoint = {
    val m = num.m
    val e = num.e
    val result   = Wire(new FixedPoint(s, d, f))
    result.r := FringeGlobals.bigIP.flt2fix(num.r, m, e, s, d, f) 
    result
  }
...
//    result.r := fNFromRecFN(m, e, fma.io.out)
  ...
  def fixexp(num: FixedPoint): FixedPoint = {
    val m = 24
    val e = 8
    val fltop   = fix2flt(num,m,e)
    val fltresult = fexp(fltop)
    val result 	= flt2fix(fltresult, num.s, num.d, num.f)
    result
  }
  ...
  def tanh(num: FloatingPoint): FloatingPoint = {
    ...
    // HACK for SVM-200 to Saturate Tanh output to +1 if num>=40; -1 if num<=-40; Tanh if -40<num<40
    val p40 = Utils.FloatPoint(m, e,  40)
    val n40 = Utils.FloatPoint(m, e, -40)
    val p1 = Utils.FloatPoint(m, e, 1)
    val n1 = Utils.FloatPoint(m, e, -1)
    val actual = (exp2t - one) /-/ (exp2t + one)
    val m1 = Wire(new FloatingPoint(m,e))
    m1 := mux( getRetimed(fabs(num) >= p40, 66.toInt) , n1 , actual ) // HACK specifically for FP32:FIXME: SG 2018/08/01
    val out = Wire(new FloatingPoint(m,e))
    out := mux( getRetimed(num >= p40, 66.toInt) , p1 , m1 ) // HACK specifically for FP32: FIXME: SG 2018/08/01
    out
  }

  def fixtanh(num: FixedPoint): FixedPoint = {
    val m = 24
    val e = 8
    val fltop   = fix2flt(num,m,e)
    val fltresult = tanh(fltop)
    val result 	= flt2fix(fltresult, num.s, num.d, num.f)
    result
  }
 ...
 //   val m = 24
 //   val e = 8
 //   val fltresult   = Wire(new FloatingPoint(m, e))
 //   fltresult.r := FringeGlobals.bigIP.fix2flt(num.r, s, d, f, m, e) 
 //   val result   = tanh(fltresult)
 //   val fixresult = Wire(new FixedPoint(s, d, f))
 //   fixresult.r := FringeGlobals.bigIP.flt2fix(result.r, m, e, s, d, f)
 //   fixresult
 // }
  ...
  def fixsigmoid(num: FixedPoint): FixedPoint = {
    val m = 24
    val e = 8
    val fltop   = fix2flt(num,m,e)
    val fltresult = sigmoid(fltop)
    val result 	= flt2fix(fltresult, num.s, num.d, num.f)
    result
  }
 ...
 // }
  ...
  def fixlog(num: FixedPoint): FixedPoint = {
    val m = 24
    val e = 8
    val fltop   = fix2flt(num,m,e)
    val fltresult = flog(fltop)
    val result 	= flt2fix(fltresult, num.s, num.d, num.f)
    result
  }
  ...
  def fixsqrt(num: FixedPoint): FixedPoint = {
    val m = 24
    val e = 8
    val fltop   = fix2flt(num,m,e)
    val fltresult = fsqrt(fltop)
    val result 	= flt2fix(fltresult, num.s, num.d, num.f)
    result
  }
  ...
  def fixrsqrt(num: FixedPoint): FixedPoint = {
    val m = 24
    val e = 8
    val fltop   = fix2flt(num,m,e)
    val fltresult = frsqrt(fltop)
    val result 	= flt2fix(fltresult, num.s, num.d, num.f)
    result
  }
  ...
  //     case d: FixedPoint => 
  //       val res = fltWire(new FixedPoint(d.s, d.d, d.f))
  ...
  def FloatPoint[T](m: Int, e: Int, init: T): FloatingPoint = {
    ...
    init match {
      case i: Int    => cst.raw := emul.FloatPoint(i, fmt).rawBitsAsInt.S.asUInt
