...
trait ChiselFileGen extends FileGen {
  ...
  /**
   * Retrieve the list of traits to split with multifile turned on.
   */
  protected def getTraitsToSplit(): List[String] = {
    (streamMapReverse.keySet.toSet.map {
      f:String => f.split('.').dropRight(1).mkString(".")  /*strip extension */
    }.toSet - "AccelTop" - "Instantiator" - "Mapping" - "IOModule")
      .filterNot(s => s.startsWith("GlobalWires") || s.startsWith("GlobalModules") || s.startsWith("Instrumentation"))
      .toList
  }
...
import chisel3.util._
""")
      open(s"trait RootController extends IOModule {")
    }
...
""")
      emit(s"""@chiselName""")
      open(s"""class GlobalWires(rc: RootController) {""")
      emit(s"""import rc._""")
    }
...
""")
      emit(s"""@chiselName""")
      open(s"""class GlobalModules(rc: RootController) {""")
      emit(s"""import rc._""")
    }
...
""")
      emit(s"""@chiselName""")
      open(s"""class Instrumentation(rc: RootController) { """)
      emit(s"""import rc._""")
    }
...
import chisel3.util._
""")
          emit(s"""trait Mixer$numMixers extends Module with ${thisTraits.mkString("\n with ")} {}""")
...
import chisel3.util._
""")
          // Have to extend `Module`, or else we may get a superclass error depending on linear trait order
          // Also note the intentional absence of newline here: global imports are inserted directly after this line
          // in ChiselCodegen.scala's emitGlobalImports.
          emit(s"""trait Mixer$numMixers extends Module with ${thisTraits.mkString("\n with ")} {}""")
