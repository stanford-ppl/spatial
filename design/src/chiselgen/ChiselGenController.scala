  ...
  */
  ...
  def createInstrumentation(lhs: Sym[Any]): Unit = {
    if (spatialConfig.enableInstrumentation) {
      ...
      emitInstrumentation(src"""io.argOuts(io_numArgOuts_reg + io_numArgIOs_reg + 2 * ${id}).valid := ${swap(lhs,En)}""")
      emitInstrumentation(src"""io.argOuts(io_numArgOuts_reg + io_numArgIOs_reg + 2 * ${id} + 1).valid := Utils.risingEdge(${swap(lhs,Done)})""")
  ...
  def emitValids(lhs: Exp[Any], cchain: Exp[CounterChain], iters: Seq[Seq[Bound[Index]]], valids: Seq[Seq[Bound[Bit]]], suffix: String = "") {
    ...
    openMethodWrapper(src"${lhs}_valids_${suffix}")
    ...
    closeMethodWrapper(src"${lhs}_valids_${suffix}")
  ...
  /** Look for the Reduce FIFO empty for the consumer(sym) of the streampipe
    * by searching for the reader of the sym.  Look for the controllers 
    * LCA of the memory to confirm it is under StreamPipe.  The memory should 
    * be an accum
    *
    * @param sym to be looked for existence of FIFO empty signal
    */
  def getReduceFIFOEmpty(sym: Exp[Any]) : String  = {
    val readMemories = readIn(sym)
    val hasStreamingRead = readMemories.exists{memory =>
        val reads = readersOf(memory).filter{read => read.ctrl.node == sym }
        reads.exists{ read =>
          val writes = getInstanceWriters(memory, read.node)
          writes.exists{write =>
            lca(write.ctrl, read.ctrl).exists(isStreamPipe) && isAccum(memory) 
          }
        }
      }
    if (hasStreamingRead) src"~${sym}_fifo_empty" else ""
  }
  ...
  def emitController(sym:Sym[Any], cchain:Option[Exp[CounterChain]], iters:Option[Seq[Bound[Index]]], isFSM: Boolean = false) {
    ...
    val symPrefix = if (isInner) "inner_" else "outer_"
    openMethodWrapper(s"${symPrefix}${sym}", prefix = "run")
    ...
    /** This code is to detect the current sym is part of the stream reduce pair
     *  The stream reduce pair required to have a memory which is an Accum and
     *  with a common LCA as StreamPipe
     *  
     *  if match, it will use to control the retime delay value (to zero)
     *  This code can be generalized to cover other stream pipe case 
     *  
     */
    val isStreamReduce = {
      val readMemories = readIn(sym)
      val writtenMemories = writtenIn(sym)
      ...
      val hasStreamingWrite = writtenMemories.exists{memory =>
        val writes = writersOf(memory).filter{write => write.ctrl.node == sym }
        writes.exists{ write =>
          val reads = getInstanceReaders(memory, write.node)
          reads.exists{read =>
            lca(write.ctrl, read.ctrl).exists(isStreamPipe) && isAccum(memory)
          }
        }
      ...
      hasStreamingRead || hasStreamingWrite
    }
    ...
    if (isStreamReduce) 
      // reduce the retime when this controller is part of stream reduce
      emitGlobalRetimeMap(src"""${sym}_retime""", s"0.toInt // stream reduce")
    else
    ...
    closeMethodWrapper(s"${symPrefix}${sym}", prefix = "run")
  }
  ...
  protected def emitChildrenCxns(sym:Sym[Any], cchain:Option[Exp[CounterChain]], iters:Option[Seq[Bound[Index]]], isFSM: Boolean = false): Unit = {
    ...
    if (!isInner) {
      ...
      val numChildrenPerMethod = 25
      val children = childrenOf(sym)
      var openIdx = 0
      children.zipWithIndex.foreach { case (c, idx) =>
        if (idx % numChildrenPerMethod == 0) {
          openIdx = idx
          openMethodWrapper(s"${sym}_child_${idx}", prefix = "run")
        }
        ...
        val fifoAddition = (if ((smStr == "Streampipe" & cchain.isDefined) ||  
                            (parentOf(sym).isDefined && isStreamPipe(parentOf(sym).get))) 
                              getReduceFIFOEmpty(c) else "") match 
                            { 
                              case "" => ""
                              case x : String => s"& ${x} /* stream reduce */"
                            }
        ...
        emit(src"""${swap(c, En)} := ${swap(c, BaseEn)} ${streamAddition} ${fifoAddition}""")  
        ...
        if ((idx+1) % numChildrenPerMethod == 0 || idx == children.length - 1) {
          closeMethodWrapper(s"${sym}_child_${openIdx}", prefix = "run")
        }
      }
    }
    /* Emit reg chains */
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    ...
    case UnitPipe(ens,func) =>
      ...
      controllerStack.push(lhs)
      openMethodWrapper(s"${lhs}_unit_pipe", aggressiveness = 2)
      if (levelOf(lhs) == OuterControl) {widthStats += childrenOf(lhs).length}
      else if (levelOf(lhs) == InnerControl) {depthStats += controllerStack.length}
      ...
      withSubStream(src"${lhs}", src"${parent_kernel}", levelOf(lhs) == InnerControl) {
        ...
        openMethodWrapper(s"$lhs", aggressiveness = 2)
        emitBlock(func)
        closeMethodWrapper(s"$lhs", aggressiveness = 2)
      }
      emitChildrenCxns(lhs, None, None)
      emitCopiedCChain(lhs)
      val en = if (ens.isEmpty) "true.B" else ens.map(quote).mkString(" && ")
      emit(src"${swap(lhs, Mask)} := $en")
      closeMethodWrapper(s"${lhs}_unit_pipe", aggressiveness = 2)
      controllerStack.pop()
    ...
    case ParallelPipe(ens,func) =>
      ...
      val parent_kernel = controllerStack.head
      controllerStack.push(lhs)
      openMethodWrapper(s"${lhs}_parallel_pipe", aggressiveness = 2)
      if (levelOf(lhs) == OuterControl) {widthStats += childrenOf(lhs).length}
      else if (levelOf(lhs) == InnerControl) {depthStats += controllerStack.length}
      emitController(lhs, None, None)
      emit(src"""${swap(lhs, CtrTrivial)} := ${DL(swap(controllerStack.tail.head, CtrTrivial), 1, true)} | false.B""")
      ...
      val en = if (ens.isEmpty) "true.B" else ens.map(quote).mkString(" && ")
      emit(src"${swap(lhs, Mask)} := $en")
      closeMethodWrapper(s"${lhs}_parallel_pipe", aggressiveness = 2)
      controllerStack.pop()
