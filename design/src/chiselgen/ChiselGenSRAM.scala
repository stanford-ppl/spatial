  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    ...
    case ParSRAMLoad(sram,inds,ens) =>
      ...
      if (dispatch.toList.length == 1) {
        ...
        val fanout = portBroadcastFanout(lhs, sram)

        emit(src"""val $lhs = Wire(${newWire(lhs.tp)})""")

        if (broadcastOf(lhs,sram,k) == 0) {
          val isBroadcaster = fanout > 1

          openMethodWrapper(s"${lhs}_par_sram_load")
          val parent = readersOf(sram).find{_.node == lhs}.get.ctrlNode
          ...
          closeMethodWrapper(s"${lhs}_par_sram_load")
          ...
          if (isBroadcaster) {
            val width = bitWidth(lhs.tp.typeArguments.head)
            val fanout = spatialConfig.target.fanoutLatencyOfSRAM(lhs,sram).toInt
            emitGlobalWireMap(src"${lhs}_broadcast", src"Wire(${newWire(lhs.tp)})")
            emit(s"""(0 until $rPar).foreach{i => ${quote(lhs)}_broadcast(i).r := Utils.getRetimed(${quote(sram)}_$k.io.output.data(${quote(lhs)}_base+i),$fanout) }""")
            emit(s"""(0 until $rPar).foreach{i => ${quote(lhs)}(i).r := ${quote(lhs)}_broadcast(i).r }""")
          }
          else
            emit(s"""(0 until $rPar).foreach{i => ${quote(lhs)}(i).r := ${quote(sram)}_$k.io.output.data(${quote(lhs)}_base+i) }""")
        }
        else {
          val broadcast = getPortBroadcaster(lhs,sram).getOrElse{ throw new Exception(s"No broadcaster for broadcast read $lhs of $sram $k") }.node

          emit(s"""(0 until $rPar).foreach{i => ${quote(lhs)}(i).r := ${quote(broadcast)}_broadcast(i).r }""")
        }
      }
      else {
        ...
        dispatch.zipWithIndex.foreach{ case (k,id) => 
          if (broadcastOf(lhs,sram,k) == 0) {
            ...
            val isBroadcaster = readersOf(sram).exists{read => broadcastOf.get(read.node,sram,k).exists{b => b > 0}}
            if (isBroadcaster && id == 0) {
              emitGlobalWireMap(src"${lhs}_broadcast", src"Wire(${newWire(lhs.tp)})")
            }
            if (isBroadcaster) {
              emit(s"""${quote(lhs)}_broadcast($id).r := ${quote(sram)}_$k.io.output.data(${quote(lhs)}_base_$k)""")
            }
          }
          else {
            val bcast = readersOf(sram).find{read => broadcastOf.get(read.node,sram,k).contains(0) }
            if (bcast.isEmpty) throw new Exception(s"No broadcaster for broadcast read $lhs of $sram $k")
            val broadcast = bcast.get.node
            emit(s"""${quote(lhs)}($id).r := ${quote(broadcast)}_broadcast($id).r """)
          }
        }
      }
      disableSplit = false
    ...
    case ParSRAMStore(sram,inds,data,ens) =>
      ...
      openMethodWrapper(s"${lhs}_par_sram_store")
      ...
      closeMethodWrapper(s"${lhs}_par_sram_store")
