...
object Number {
  ...
  def ln(x: FixedPoint): FixedPoint = FixedPoint(Math.log(x.toDouble), x.fmt).withValid(x.valid)
  def log2(x: FixedPoint): FixedPoint = FixedPoint(Math.log(x.toDouble) / Math.log(2), x.fmt).withValid(x.valid)
