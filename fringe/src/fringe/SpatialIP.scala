...
/** Top module including Fringe and Accel (see target specific for fringe <-> accel connections)
  ...
  */
class SpatialIP(targetName: String, accelGen: () => AbstractAccelUnit)
    extends Module {
  globals.target = targetName match {
    ...
    case "de1"                   => new targets.de1.DE1
    case _                       => throw new Exception(s"Unknown target '$targetName'")
  }

  globals.target.makeIO = { x: Data =>
    IO(x)
  }
  val accel: AbstractAccelUnit = accelGen()
  accel.io <> DontCare
  val io: SpatialIPInterface = globals.target.addFringeAndCreateIP(reset, accel)
