...
class SystolicArray2D(val dims: List[Int], val neighborhood_size: List[Int], val movement_scalars: List[Double], val self_position: List[Int],
  ...
  assert(self_position.count(_!=0) == 1)
  ...
  */
  ...
  val vertex_thickness = (0 until 4).map{p => 
    ...
    val has_nonzero = (dim0_start until dim0_end).flatMap{i => (dim1_start until dim1_end).map{j =>
  ...
  */
  ...
  (0 until dims(0)).foreach{ i =>
    (0 until dims(1)).foreach{ j => 
      ...
      val new_val = (0 until neighborhood_size(0)).flatMap{ ii => (0 until neighborhood_size(1)).map{ jj =>
        ...
        if (weight != 0 & !mac_ignore_self) {
          ...
          operation match {
            case Sum | Min | Max => 0.FP(true, bitWidth-fracBits, fracBits)
            case MAC | Product   => 1.FP(true, bitWidth-fracBits, fracBits)
          }
        }

      }}.reduce{ (a,b) => operation match {
        case Sum      => a + b
        case MAC      => a * b
        case Product  => a * b
