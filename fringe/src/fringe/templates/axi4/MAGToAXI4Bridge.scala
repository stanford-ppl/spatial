...
class MAGToAXI4Bridge(val p: AXI4BundleParameters) extends Module {
  // TODO: Why is this assert here since not all targets use 512 bus width?
  Predef.assert(
    p.dataBits == globals.EXTERNAL_W * globals.EXTERNAL_V,
    s"ERROR: Unsupported data width ${p.dataBits} in MAGToAXI4Bridge (Target expects ${globals.EXTERNAL_W} * ${globals.EXTERNAL_V}"
  )

  val io = IO(new Bundle {
    val in = Flipped(new DRAMStream(
      globals.DATA_WIDTH,
      globals.WORDS_PER_STREAM)) // hardcoding stuff here
    val M_AXI = new AXI4Inlined(p)
  })

  io <> DontCare
  scala.Console.println(
    s"M_AXI AWADDR width = ${io.M_AXI.AWADDR.getWidth}, cmd.bits.addr width = ${io.in.cmd.bits.addr.getWidth}")
  ...
  io.in.cmd.ready := Mux(io.in.cmd.bits.isWr,
                         io.M_AXI.AWREADY,
                         io.M_AXI.ARREADY) // Used to be shift registered
