  ...
  if (par > 0) {
    ...
    bases.zipWithIndex.foreach{ case (b,i) =>
      ...
      b.io.xBarW(0).reset.head := io.input.reset |
        ...
        {if (start.isDefined) false.B else {!(locked | io.input.enable) && (getRetimed(io.input.start,1) =/= io.input.start)}}
    ...
    (0 until par).foreach{ i =>
      ...
      val c = defs match {
        ...
        case 0x3 | 0x2 | 0x1 | 0x0 => Mux(!locked && (getRetimed(io.input.start,1) =/= io.input.start), inits(i), counts(i))
      }
