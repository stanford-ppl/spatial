...
abstract class LazyModule
{
  ...
  def moduleName: String = className + valName.orElse(outerName).map("_" + _).getOrElse("")
  def instanceName: String = valName.getOrElse(outerName.map(_ + "_").getOrElse("") + className)
  def name: String = valName.getOrElse(className)
  def line: String = sourceLine(info)

  def module: LazyModuleImp

  protected[diplomacy] def instantiate(): Unit = {
  ...
  def omitGraphML: Boolean = nodes.isEmpty && children.isEmpty
...
object LazyModule {
  ...
  def apply[T <: LazyModule](bc: T)(implicit sourceInfo: SourceInfo): T = {
    ...
    require (stack.nonEmpty, s"LazyModule() applied to ${bc.name} twice ${sourceLine(sourceInfo)}")
...
abstract class LazyModuleImp(outer: LazyModule) extends Module {
  ...
  override def desiredName: String = outer.moduleName
