...
case class IdRange(start: Int, end: Int) {
  ...
  def <(x: IdRange): Boolean = end <= x.start
  def >(x: IdRange): Boolean = x < this
...
case class AddressSet(base: BigInt, mask: BigInt) extends Ordered[AddressSet] {
  ...
  def contains(x: UInt): Bool = ((x ^ UInt(base)).zext() & SInt(~mask)) === SInt(0)
  ...
  def max: BigInt = { require(finite); base | mask }
