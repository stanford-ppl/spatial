...
class DRAMArbiter(
  ...
  val io = IO(new Bundle {
    ...
    val app = new AppStreams(LOAD_STREAMS,
                             STORE_STREAMS,
                             GATHER_STREAMS,
                             SCATTER_STREAMS)
  ...
  val signalLabels = if (numStreams > 0) {
    ...
    val loadControllers = loadStreamInfo.zipWithIndex.map {
      case (s, i) =>
    ...
    val storeControllers = storeStreamInfo.zipWithIndex.map {
      case (s, i) =>
    ...
    val gatherControllers = gatherStreamInfo.zipWithIndex.map {
      case (s, i) =>
    ...
    val scatterControllers = scatterStreamInfo.zipWithIndex.map {
      case (s, i) =>
    ...
    if (isDebugChannel) {
      ...
      def connectDbgSig(sig: UInt, label: String): Unit = {
        if (globals.enableDebugRegs) {
          if (isDebugChannel) {
            ...
            val padded_label = if (label.length < 55) {
              label + "." * (55 - label.length)
            } else label
      ...
      connectDbgSig(
        debugCounter(io.dram.wdata.valid),
        " # WVALID"
      )
      connectDbgSig(
        debugCounter(io.dram.wdata.ready),
        " # WREADY"
      )
      connectDbgSig(
        debugCounter(io.dram.cmd.ready & io.dram.cmd.bits.isWr),
        " # AWREADY"
      )
      connectDbgSig(
        debugCounter(io.dram.cmd.valid & io.dram.cmd.bits.isWr),
        " # AWVALID"
      )
      connectDbgSig(
        debugCounter(io.dram.wresp.valid),
        " # BVALID"
      )
      connectDbgSig(
        debugCounter(io.dram.wresp.ready),
        " # BREADY"
      )

      val rdataEnqCount = debugCounter(
        io.dram.rresp.valid & io.dram.rresp.ready)
      val wdataCount = debugCounter(io.dram.wdata.valid & io.dram.wdata.ready)
      val appWdataCount = debugCounter(
        io.app.stores.head.data.valid & io.app.stores.head.data.ready)
      connectDbgSig(wdataCount,
                    "[FRINGE] num wdata transferred (wvalid & wready)")

      connectDbgSig(debugCounter(io.dram.cmd.ready & io.dram.cmd.valid),
                    "[FRINGE] # DRAM Commands Issued")
      connectDbgSig(
        debugCounter(
          io.dram.cmd.ready & io.dram.cmd.valid & !io.dram.cmd.bits.isWr),
        "[FRINGE] # Read Commands Sent")
      connectDbgSig(
        debugCounter(
          io.dram.cmd.ready & io.dram.cmd.valid & io.dram.cmd.bits.isWr),
        "[FRINGE] # Write Commands Sent")

      // Count number of load commands issued from accel per stream
      io.app.loads.zipWithIndex.foreach {
        case (load, i) =>
          val loadCounterHandshake =
            debugCounter(io.enable & load.cmd.valid & load.cmd.ready)
      ...
      io.app.stores.zipWithIndex.foreach {
        case (store, i) =>
          val storeCounterHandshake =
            debugCounter(io.enable & store.cmd.valid & store.cmd.ready)
          connectDbgSig(storeCounterHandshake,
                        s"[ACCEL] # from store stream $i")
      }

      connectDbgSig(debugCounter(io.dram.rresp.valid & io.dram.rresp.ready),
                    "[FRINGE] # Read Responses Acknowledged")
      connectDbgSig(debugCounter(io.dram.rresp.valid & !io.dram.rresp.ready),
                    "[FRINGE] # RResp rejected by ready")
      connectDbgSig(debugCounter(!io.dram.rresp.valid & io.dram.rresp.ready),
                    "[FRINGE] Cycles RResp ready and idle (~valid)")
      loadStreamInfo.indices.foreach { i =>
        val signal = s"[FRINGE]  # from load stream $i"
        connectDbgSig(
          debugCounter(
            io.dram.rresp.valid & io.dram.rresp.ready & (io.dram.rresp.bits.getTag.streamID === i.U)),
          signal)
      }
      connectDbgSig(debugCounter(io.dram.wresp.valid & io.dram.wresp.ready),
                    "[FRINGE] # Write Responses Acknowledged")
      connectDbgSig(debugCounter(io.dram.wresp.valid & !io.dram.wresp.ready),
                    "[FRINGE] # WResp rejected by ready")
      connectDbgSig(debugCounter(!io.dram.wresp.valid & io.dram.wresp.ready),
                    "[FRINGE] Cycles WResp ready and idle (~valid)")
      storeStreamInfo.indices.foreach { i =>
        val signal = s"[FRINGE]  # from store stream $i"
        connectDbgSig(
          debugCounter(
            io.dram.wresp.valid & io.dram.wresp.ready & (io.dram.wresp.bits.getTag.streamID === (i + loadStreamInfo.length).U)),
          signal)
      }

      // Connect fringe io.dram interface debuggers
      connectDbgSig(
        debugFF(io.dram.cmd.bits.addr, io.dram.cmd.valid && io.dram.cmd.ready),
        "Last io.dram cmd (leaving fringe)")
      connectDbgSig(
      ...
      connectDbgSig(debugFF(io.dram.wdata.bits.wstrb.head,
                            io.dram.wdata.valid & io.dram.wdata.ready),
                    "Last io.dram wstrb.head (leaving fringe)")
      connectDbgSig(
        debugFF(io.dram.wdata.bits.wdata.head,
                io.dram.wdata.valid & io.dram.wdata.ready & wdataCount === 0.U),
        "First io.dram wdata.head (leaving fringe)")
      connectDbgSig(
        debugFF(io.dram.wdata.bits.wstrb.head,
                io.dram.wdata.valid & io.dram.wdata.ready & wdataCount === 0.U),
        "First io.dram wstrb.head (leaving fringe)")
      connectDbgSig(
        debugFF(io.dram.wdata.bits.wdata.head,
                io.dram.wdata.valid & io.dram.wdata.ready & wdataCount === 1.U),
        "Second io.dram wdata.head (leaving fringe)")
      connectDbgSig(
        debugFF(io.dram.wdata.bits.wstrb.head,
                io.dram.wdata.valid & io.dram.wdata.ready & wdataCount === 1.U),
        "Second io.dram wstrb.head (leaving fringe)")
      connectDbgSig(
        debugFF(io.app.stores.head.cmd.bits.addr,
                io.app.stores.head.cmd.valid && io.app.stores.head.cmd.ready),
        "Last store cmd (entering fringe)")
      connectDbgSig(
        debugFF(io.app.stores.head.cmd.bits.size,
                io.app.stores.head.cmd.valid && io.app.stores.head.cmd.ready),
        "Last store size (entering fringe)")
      connectDbgSig(
        debugFF(io.app.stores.head.data.bits.wdata.head,
                io.app.stores.head.data.valid & io.app.stores.head.data.ready),
        "Last store wdata.head (entering fringe)"
      )
      connectDbgSig(
        debugFF(io.app.stores.head.data.bits.wstrb,
                io.app.stores.head.data.valid & io.app.stores.head.data.ready),
        "Last store wstrb.head (entering fringe)")
      connectDbgSig(
        debugFF(
          io.app.stores.head.data.bits.wdata.head,
          io.app.stores.head.data.valid & io.app.stores.head.data.ready & appWdataCount === 0.U),
        "First store wdata.head (entering fringe)"
      )
      connectDbgSig(
        debugFF(
          io.app.stores.head.data.bits.wstrb,
          io.app.stores.head.data.valid & io.app.stores.head.data.ready & appWdataCount === 0.U),
        "First store wstrb.head (entering fringe)"
      )
      connectDbgSig(
        debugFF(
          io.app.stores.head.data.bits.wdata.head,
          io.app.stores.head.data.valid & io.app.stores.head.data.ready & appWdataCount === 1.U),
        "Second store wdata.head (entering fringe)"
      )
      connectDbgSig(
        debugFF(
          io.app.stores.head.data.bits.wstrb,
          io.app.stores.head.data.valid & io.app.stores.head.data.ready & appWdataCount === 1.U),
        "Second store wstrb.head (entering fringe)"
      )
...
//                    "# cycles TOP ARREADY & ARVALID ")
      ...
      connectDbgSig(debugCounter(io.TOP_AXI.RREADY & io.TOP_AXI.RVALID),
                    "# cycles TOP RREADY & RVALID ")
      connectDbgSig(debugCounter(io.TOP_AXI.WVALID), "# cycles TOP WVALID ")
      connectDbgSig(debugCounter(io.TOP_AXI.WREADY & io.TOP_AXI.WVALID),
                    "# cycles TOP WREADY & WVALID ")
      connectDbgSig(debugCounter(!io.TOP_AXI.WREADY & io.TOP_AXI.WVALID),
                    "# cycles TOP ~WREADY & WVALID (forced)")
      ...
      connectDbgSig(
        debugFF(io.TOP_AXI.ARADDR, io.TOP_AXI.ARVALID & io.TOP_AXI.ARREADY),
        "Last TOP ARADDR")
      connectDbgSig(
        debugFF(io.TOP_AXI.ARLEN, io.TOP_AXI.ARVALID & io.TOP_AXI.ARREADY),
        "Last TOP ARLEN")
      connectDbgSig(
        debugFF(io.TOP_AXI.ARSIZE, io.TOP_AXI.ARVALID & io.TOP_AXI.ARREADY),
        "Last TOP ARSIZE")
      connectDbgSig(
        debugFF(io.TOP_AXI.ARID, io.TOP_AXI.ARVALID & io.TOP_AXI.ARREADY),
        "Last TOP ARID")
      connectDbgSig(
        debugFF(io.TOP_AXI.ARBURST, io.TOP_AXI.ARVALID & io.TOP_AXI.ARREADY),
        "Last TOP ARBURST")
      connectDbgSig(
        debugFF(io.TOP_AXI.AWADDR, io.TOP_AXI.AWVALID & io.TOP_AXI.AWREADY),
        "Last TOP AWADDR")
      connectDbgSig(
        debugFF(io.TOP_AXI.AWLEN, io.TOP_AXI.AWVALID & io.TOP_AXI.AWREADY),
        "Last TOP AWLEN")
      connectDbgSig(
        debugFF(io.TOP_AXI.WDATA, io.TOP_AXI.WVALID & io.TOP_AXI.WREADY),
        "Last TOP WDATA")
      connectDbgSig(
        debugFF(io.TOP_AXI.WSTRB, io.TOP_AXI.WVALID & io.TOP_AXI.WREADY),
        "Last TOP WSTRB")
      connectDbgSig(
        debugFF(io.TOP_AXI.WDATA,
                io.TOP_AXI.WVALID & io.TOP_AXI.WREADY & wdataCount === 0.U),
        "First TOP WDATA")
      connectDbgSig(
        debugFF(io.TOP_AXI.WSTRB,
                io.TOP_AXI.WVALID & io.TOP_AXI.WREADY & wdataCount === 0.U),
        "First TOP WSTRB")
      connectDbgSig(
        debugFF(io.TOP_AXI.WDATA,
                io.TOP_AXI.WVALID & io.TOP_AXI.WREADY & wdataCount === 1.U),
        "Second TOP WDATA")
      connectDbgSig(
        debugFF(io.TOP_AXI.WSTRB,
                io.TOP_AXI.WVALID & io.TOP_AXI.WREADY & wdataCount === 1.U),
