...
import fringe.templates.counters.{SingleCounter, NBufCtr}
...
class LineBuffer(
  val num_lines: Int,
  val line_size: Int,
  val empty_stages_to_buffer: Int,
  val rstride: Int,
  val col_wPar: Int,
  val col_rPar:Int,
  val col_banks: Int,
  val numWriters: Int,
  val row_rPar: Int,
  val transientPar: Int,
  val numAccessors: Int,
  val bitWidth: Int = 32
) extends Module {
  ...
  val wCRN_width = 1 + log2Up(num_lines+extra_rows_to_buffer)
  ...
  val READ_countRowNum = (0 until row_rPar).map{ i => 
    val c = Module(new NBufCtr(rstride, Some(rstride+i), Some(num_lines+extra_rows_to_buffer), 1 + log2Up(num_lines+extra_rows_to_buffer)))
  ...
  def readRow(row: UInt): UInt = { 
    val readableData = (0 until row_rPar).map{i => i.U -> io.data_out(i) }
    MuxLookup(row, 0.U,  readableData)
  }
  def readRowSlice(row: UInt, relative_col: UInt): UInt = { 
    val readableData = (0 until row_rPar * col_rPar).map{i => i.U -> io.data_out(i) }
    MuxLookup(row.*-*(col_rPar.U, None, true.B) + relative_col, 0.U,  readableData)
  }

  def readRow(row: Int): UInt = io.data_out(row)
