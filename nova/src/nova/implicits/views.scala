...
object views {
  implicit class ViewOps[A](x: A) {
    ...
    def viewRef(implicit tp: Type[A]): Ref[A] = x match {
      case t: Ref[_] if t.tp <:< tp && !t.isType => x.asInstanceOf[Ref[A]]
      case t: Ref[_] if t.isType => throw new Exception(s"Cannot view $x as a Top[$tp] ($x is a Type)")
      case t: Ref[_] => throw new Exception(s"Cannot view $x as a Top[$tp] ($x has type ${t.tp})")
