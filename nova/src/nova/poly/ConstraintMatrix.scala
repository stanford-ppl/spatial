...
case class ConstraintMatrix(rows: Set[SparseConstraint], hasDomain: Boolean = false) {
  ...
  @stateful def domain: ConstraintMatrix = keys.map{k => domainOf(k)}.fold(ConstraintMatrix.empty){_::_}
