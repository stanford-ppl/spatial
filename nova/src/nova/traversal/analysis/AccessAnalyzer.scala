...
case class AccessAnalyzer(IR: State) extends Traversal with AccessExpansion {
  ...
  /**
    ...
    */
  private def isAllInvariant(is: Seq[Idx], xs: Seq[Sym[_]]): Boolean = xs.forall{x => is.forall{i => isInvariant(i,x) }}

  /**
    * Returns the innermost iterator which the symbols in xs vary with.
    */
  private def lastVariantIter(is: Seq[Idx], x: Sym[_]): Option[Idx] = is.reverseIterator.find{i => !isInvariant(i,x) }

  object Plus  { def unapply(x: Idx): Option[(Idx,Idx)] = x.op.collect{case FixAdd(a,b) => (a,b) }}
  object Minus { def unapply(x: Idx): Option[(Idx,Idx)] = x.op.collect{case FixSub(a,b) => (a,b) }}
  object Times { def unapply(x: Idx): Option[(Idx,Idx)] = x.op.collect{case FixMul(a,b) => (a,b) }}
  object Index { def unapply(x: Idx): Option[Idx] = Some(x).filter(iters.contains) }

  private lazy val Zero = Sum.single(0)
  private lazy val One  = Prod.single(1)
  private def stride(i: Idx): Prod = if (ctrOf.get(i).isDefined) Prod.single(i.ctrStep) else One
