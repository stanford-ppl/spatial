...
class NBufCtr(val stride: Int = 1, val start: Option[Int], val stop: Option[Int], 
             val width: Int = 32, val countDir: Int = 0) extends Module {
  ...
  if (start.isDefined && stop.isDefined) {
    ...
    if (countDir == 0) {
      ...
      val nextCntUp = Mux(io.input.enable, Mux(cnt + stride.U(width.W) >= stop.get.U(width.W), 0.U(width.W) + (cnt.asSInt + (stride - stop.get).S(width.W)).asUInt, cnt+stride.U(width.W)), cnt)
      cnt := Utils.getRetimed(Mux(reset.toBool, 0.U, nextCntUp), 1)            
    } else {
      val nextCntDown = Mux(io.input.enable, Mux(cnt === 0.U(width.W), (stop.get-stride).U(width.W), cnt-stride.U(width.W)), cnt) // TODO: This could be an issue if strided counter is used in reverse
      cnt := Utils.getRetimed(Mux(reset.toBool, 0.U, nextCntDown), 1)            
    }
    ...
    if (countDir == 0) {
      ...
      val nextCntUp = Mux(io.input.enable, Mux(cnt + stride.U(width.W) >= stop.get.U(width.W), 0.U(width.W) + cnt+stride.U(width.W) - stop.get.U(width.W), cnt+stride.U(width.W)), cnt)
      cnt := Utils.getRetimed(Mux(reset.toBool, 0.U(width.W), nextCntUp), 1)
    } else {
      val nextCntDown = Mux(io.input.enable, Mux(cnt === 0.U(width.W), (stop.get-stride).U(width.W), cnt-stride.U(width.W)), cnt) // TODO: This could be an issue if strided counter is used in reverse
      cnt := Utils.getRetimed(Mux(reset.toBool, 0.U(width.W), nextCntDown), 1)
    }
...
class SingleCounter(val par: Int, val start: Option[Int], val stop: Option[Int], 
  ...
  // Chisel3 seems to define minWidth (protected def) as 1+ lit.bitLength,
  // and errors if width is lesser than this value.
  // Manually set the width appropriately, at the expense of ugliness
  def getS(x: Int, w: Int) = {
    x.S(math.max(BigInt(x).bitLength + 1, w).W)
  }
  ...
  if (par > 0) {
    ...
    val inits = List.tabulate(par){i => 
      Utils.getRetimed(
        if (start.isDefined & stride.isDefined) getS(start.get + i*stride.get, width)
        else if (start.isDefined) {getS(start.get, width) + getS(i, width) * io.input.stride}
        else if (stride.isDefined) {io.input.start + getS(i*stride.get, width)}
        else {io.input.start + getS(i, width) * io.input.stride}
    ...
    val delta = if (stride.isDefined & gap.isDefined) { getS(stride.get*par+gap.get, width) }
                else if (stride.isDefined) { getS(stride.get*par, width) + io.input.gap}
                else if (gap.isDefined) { io.input.stride * getS(par, width) + getS(gap.get, width)}
                else { io.input.stride * getS(par, width) + io.input.gap}
    val newvals = counts.map( _ + delta)
    val isMax = Mux(io.input.stride >= getS(0, width), 
      newvals(0) >= {if (stop.isDefined) getS(stop.get, width) else io.input.stop}, 
      newvals(0) <= {if (stop.isDefined) getS(stop.get, width) else io.input.stop}
    )
    ...
    if(stride.isDefined) {
      (0 until par).foreach { i => 
        ...
        io.output.countWithoutWrap(i) := Mux(counts(i) === getS(0, width), if(stop.isDefined) getS(stop.get, width) else io.input.stop, counts(i))
      }
    } else {
      (0 until par).foreach { i => 
        io.output.count(i) := counts(i)
    ...
    io.output.count(0) := getS(0, width)
