...
class FixedPoint(val s: Boolean, val d: Int, val f: Int) extends Bundle {
	...
	def toFloat(m_out: Int, e_out: Int): FloatingPoint = {
		val result = Wire(new FloatingPoint(m_out, e_out))
		result.r := FringeGlobals.bigIP.fix2flt(this.r, s, d, f, m_out, e_out)
		result
	}

//	def toFixed(sign: Boolean, dec: Int, frac: Int): FixedPoint = {
//		val result = Wire(new FixedPoint(sign,dec,frac))
//		result.r := FringeGlobals.bigIP.flt2fix(this.r, m, e, sign, dec, frac)
//    		result
//	}
...
class FloatingPoint(val m: Int, val e: Int) extends Bundle {
	...
	def +[T] (rawop: T, rounding:LSBCasting = Truncate, saturating:MSBCasting = Lazy): FloatingPoint = {
		rawop match {
			case op: FloatingPoint => 
                ...
                result.r := FringeGlobals.bigIP.fadd(this.r, op.r, m, e, 12)
				result
		}
	}
	...
	def -[T] (rawop: T, rounding:LSBCasting = Truncate, saturating:MSBCasting = Lazy): FloatingPoint = {
		rawop match {
			case op: FloatingPoint => 
				assert(op.m == m & op.e == e)
				val result = Wire(new FloatingPoint(m, e))
				result.r := FringeGlobals.bigIP.fsub(this.r, op.r, m, e)
				result
		}
	}
	...
	def *-*[T] (rawop: T, rounding:LSBCasting = Truncate, saturating:MSBCasting = Lazy): FloatingPoint = {
		rawop match {
			case op: FloatingPoint => 
				assert(op.m == m & op.e == e, s"Mismatched floating point types: (${m},${e}) and (${op.m},${op.e})")
				val result = Wire(new FloatingPoint(m, e))
				result.r := FringeGlobals.bigIP.fmul(this.r, op.r, m, e)
				result
		}
	}
	...
	def /-/[T] (rawop: T, rounding:LSBCasting = Truncate, saturating:MSBCasting = Lazy): FloatingPoint = {
		rawop match {
			case op: FloatingPoint => 
				assert(op.m == m & op.e == e)
				val result = Wire(new FloatingPoint(m, e))
				result.r := FringeGlobals.bigIP.fdiv(this.r, op.r, m, e)
				result
		}
	}
	...
	def <[T] (rawop: T): Bool = {
        ...
        result := FringeGlobals.bigIP.flt(this.r, op.r, m, e)
				result
		}
	}
	...
	def ===[T] (rawop: T): Bool = {
		rawop match {
			case op: FloatingPoint => 
				assert(op.m == m & op.e == e)
				val result = Wire(Bool())
        result := FringeGlobals.bigIP.feq(this.r, op.r, m, e)
				result
		}
	}
	...
	def >[T] (rawop: T): Bool = {
		rawop match {
			case op: FloatingPoint => 
				assert(op.m == m & op.e == e)
				val result = Wire(Bool())
        result := FringeGlobals.bigIP.fgt(this.r, op.r, m, e)
				result
		}
	}
	...
	def >=[T] (rawop: T): Bool = {
		rawop match {
			case op: FloatingPoint => 
				assert(op.m == m & op.e == e)
				val result = Wire(Bool())
        result := FringeGlobals.bigIP.fge(this.r, op.r, m, e)
				result
		}
	}
	...
	def <=[T] (rawop: T): Bool = {
		rawop match {
			case op: FloatingPoint => 
				assert(op.m == m & op.e == e)
				val result = Wire(Bool())
        result := FringeGlobals.bigIP.fle(this.r, op.r, m, e)
				result
		}
	}
	...
	def =/=[T] (rawop: T): Bool = {
		rawop match {
			case op: FloatingPoint => 
				assert(op.m == m & op.e == e)
				val result = Wire(Bool())
        result := FringeGlobals.bigIP.fne(this.r, op.r, m, e)
				result
		}
	}
	...
	def FltFMA[T] (rawop1: T, rawop2: T, rounding:LSBCasting = Truncate, saturating:MSBCasting = Lazy): FloatingPoint = {
		(rawop1, rawop2) match {
			case (op1: FloatingPoint, op2: FloatingPoint) => 
				assert(op1.m == m & op1.e == e, "Floating point types not the same!")
				assert(op2.m == m & op2.e == e, "Floating point types not the same!")
				val result = Wire(new FloatingPoint(m, e))
				result.r := FringeGlobals.bigIP.ffma(this.r, op1.r, op2.r, m, e)
				result
		}
	}
	...
	def toFloat(m_out: Int, e_out: Int): FloatingPoint = {
		val result = Wire(new FloatingPoint(m_out, e_out))
		result.r := FringeGlobals.bigIP.flt2flt(this.r, m, e, m_out, e_out)
		result
	}
