...
for line in src:
  if 'int main' in line:  # TODO: Use regex, or just sed on cmd line
    ...
    dst.write(line.replace('std::to_string', 'convert_to_string_int32_t').replace(');', ',tmp_buf' + str(global_buf_count) + ');'))
    global_buf_count += 1
  elif 'string_plus' in line:
    dst.write(line.replace('string_plus(', '').replace(', ', ' + ').replace(');', ';'))
  elif ' = std::string(argv[i]);' in line:
    dst.write(line.replace(' = std::string(argv[i]);', ' = std::string(argv[i]);', 1))
