...
python3 ${REGRESSION_HOME}/spatial/spatial/utilities/gdocs.py "report_synth_results" $appname "$lutraw (${lutpcnt}%)" "$regraw (${regpcnt}%)" "$ramraw (${rampcnt}%)" "$uramraw (${urampcnt}%)" "$dspraw (${dsppcnt}%)" "$lalraw (${lalpcnt}%)" "$lamraw (${lampcnt}%)" "$synthtime" "$tmg" "$1" "$hash" "$ahash"
...
if [[ $1 = "Zynq" ]]; then
    ...
    python3 ${REGRESSION_HOME}/spatial/spatial/utilities/gdocs.py "report_board_runtime" $appname $timeout $runtime $pass "$2 $3 $4 $5 $6 $7 $8" "$1" "$locked" "$hash" "$ahash"
elif [[ $1 = "ZCU" ]]; then
	APP=$(basename $(pwd))
