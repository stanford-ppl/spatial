...
u64 Xil_In64(u64 Addr) { return *(volatile u64 *)Addr; }
...
int fileToBuf(unsigned char *buf, char *filename, u32 max_bytes) {
  ...
  ASSERT(max_bytes > 0,
         "Trying to read invalid (%lu) number of bytes from %s\n", max_bytes,
         filename);
...
void printHelp(int argc, char **argv) {
  ...
  EPRINTF("-p <file> :  Load MIPS instruction buffer with provided textual hex "
          "file, one word per line\n");
  EPRINTF("-w        :  Get input video from webcam. Runs program in a loop "
          "for every frame\n");
  EPRINTF("-f <file> :  Get input video from file. Runs program in a loop for "
          "every frame\n");
  EPRINTF("-i <file> :  Load MIPS data input buffer with provided hex file. "
          "Atleast one of -w, -f or -i must be specified\n");
  EPRINTF("-o <file> :  Dump MIPS data output buffer into text file in hex "
