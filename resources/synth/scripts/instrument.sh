...
if [[ ! -f html/controller_tree.bak.html ]]; then
	...
	cp html/controller_tree.bak.html html/controller_tree.html 
fi
...
sed -i "s/<h2>Controller Diagram for .*<\/h2>/<h2>Controller Diagram for $appname<\/h2><h2><font color=\"red\">Instrumentation Annotations <\/font><\/h2>/g" html/controller_tree.html
x=0
while IFS='' read -r line || [[ -n "$line" ]]; do
	if [[ x -eq 0 ]]; then
		sed -i "s/Instrumentation Annotations/Instrumentation Annotations ($line)/g" html/controller_tree.html
	else
