package nova.traversal.analysis.banking
...
import forge.math.isPow2
import nova.poly.{ConstraintMatrix, ISL, SparseMatrix, SparseVector}
...
case class ExhaustiveBanking()(implicit IR: State, isl: ISL) extends BankingStrategy {
  ...
  protected def findBanking(grps: Set[Seq[SparseMatrix]], dims: Seq[Int]): ModBanking = {
    ...
    val Nmin = grps.map(_.size).maxOrElse(1)
