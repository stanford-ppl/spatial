...
trait Spatial extends Compiler with ParamLoader {
  ...
  def runPasses[R](block: Block[R]): Block[R] = {
    ...
    lazy val streamify = Seq(unitPipeToForeach) ++ bankingAnalysis ++ createDump("PreStream") ++ Seq(FlattenToStream(state), streamBufferExpansion, pipeInserter, printer, streamChecks)++ createDump("PostStream")
