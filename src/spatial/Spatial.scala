...
""")
    ...
    cli.opt[Unit]("optimizeDiv").action{(_,_) => spatialConfig.optimizeDiv = true; spatialConfig.useCrandallMod = true}.text("Replace division by Crandall's Algorithm to avoid using DSPs.  Also turns on Crandall-based modulo since the datapath will already contain most of the nodes required to replace modulo")
