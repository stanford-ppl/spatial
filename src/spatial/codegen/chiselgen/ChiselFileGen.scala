  ...
  override def emitFooter(): Unit = {
    inAccel {// Guarantee prints for this section
      ...
      inGen(out, "Instantiator.scala") {
        ...
        emit (s"""new Top("${spatialConfig.target.name}", () => Module(new AccelTop(w, numArgIns, numArgOuts, numArgIOs, numArgOuts_instr + numArgBreakpts, io_argOutLoopbacksMap, loadStreamInfo, storeStreamInfo, streamInsInfo, streamOutsInfo)))""")
      ...
      inGenn(out, "IOModule", ext) {
        ...
        open("val io = IO(new CustomAccelInterface(io_w, io_v, globals.LOAD_STREAMS, globals.STORE_STREAMS, io_numArgIns, io_numArgOuts, io_numArgOutLoopbacks))")
  //         emit ("")
  //         emit ("// Stream IO")
