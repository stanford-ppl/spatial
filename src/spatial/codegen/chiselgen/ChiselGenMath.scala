...
trait ChiselGenMath extends ChiselGenCommon {
  ...
  private def MathDL(lhs: Sym[_], rhs: Op[_], nodelat: Double): Unit = {
    ...
    rhs match {
      case FixMul(x,y) => emit(src"""$lhs.r := ($x.mul($y, $lat, $backpressure, Truncate, Wrapping, "$lhs")).r""")
      case UnbMul(x,y) => emit(src"""$lhs.r := ($x.mul($y, $lat, $backpressure, Unbiased, Wrapping, "$lhs")).r""")
      case SatMul(x,y) => emit(src"""$lhs.r := ($x.mul($y, $lat, $backpressure, Truncate, Saturating, "$lhs")).r""")
      case UnbSatMul(x,y) => emit(src"""$lhs.r := ($x.mul($y, $lat, $backpressure, Unbiased, Saturating, "$lhs")).r""")
      case FixDiv(x,y) => emit(src"""$lhs.r := ($x.div($y, $lat, $backpressure, Truncate, Wrapping, "$lhs")).r""")
      case UnbDiv(x,y) => emit(src"""$lhs.r := ($x.div($y, $lat, $backpressure, Unbiased, Wrapping, "$lhs")).r""")
      case SatDiv(x,y) => emit(src"""$lhs.r := ($x.div($y, $lat, $backpressure, Truncate, Saturating, "$lhs")).r""")
      case UnbSatDiv(x,y) => emit(src"""$lhs.r := ($x.div($y, $lat, $backpressure, Unbiased, Saturating, "$lhs")).r""")
      case FixMod(x,y) => emit(src"""$lhs.r := ($x.mod($y, $lat, $backpressure, "$lhs")).r""")
      case FixRecip(x) => emit(src"""$lhs.r := (${lhs}_one.div($x, $lat, $backpressure, "$lhs")).r""")
