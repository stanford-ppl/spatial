...
case class ResourceReporter(IR: State)  extends NamedCodegen with FileDependencies with AccelTraversal {
  ...
  override def emitHeader(): Unit = { 
    super.emitHeader()
    emit("#!/bin/python")
    open(s"def compSize(bitWidth, dims, padding, nBuf):")
    emit(s"size = product([d + p for d,p in zip(dims,padding)])")
    emit(s"return size * bitWidth")
    close(s"")
    emit("bramSize = {} # in bits")
    emit("regSize = {} # in bits")
  }

  override def emitFooter(): Unit = { 
    emit(s"""print("total fix op": $fixOp")""")
    super.emitFooter()
  }

  def emitMem(lhs:Sym[_], tp:String, dims:Seq[Int], padding:Seq[Int], depth:Int) = {
    emit(src"""${tp}Size["${lhs}"] = compSize(${bitWidth(lhs.tp.typeArgs.head)}, ${dims}, ${padding}, ${depth})""")
  }
