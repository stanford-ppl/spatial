...
case class TreeGen(IR: State)
    extends AccelTraversal
    with argon.codegen.Codegen {
  ...
  val memColors = Seq(
    ...
    "efc11b",
    "aed919"
  ) // List of colors I think looks nice
  ...
  override def gen(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    ...
    case _: MemAlloc[_, _]
        if inHw && (lhs.isSRAM | lhs.isRegFile | lhs.isReg | lhs.isLineBuffer | lhs.isFIFOReg | lhs.isFIFO | lhs.isLIFO) =>
      logMem(lhs, rhs)
    case _ =>
  ...
  def inCell(cellName: String, hasCollapsible: Boolean)(header: => Unit)(
      collapsible: => Unit): Unit = {
  ...
  def inTitledCollapsible(hasTitle: Boolean)(title: => Unit)(
      inside: => Unit): Unit = {
    if (hasTitle) {
    ...
    emit(
      s"""${"  " * ident}<TABLE BORDER="3" CELLPADDING="10" CELLSPACING="10">""")
  ...
  def inCollapsible(inside: => Unit): Unit = {
    inTitledCollapsible(false)(())(inside)
  }
  ...
  protected def link(s: String): String =
    s"""<a href=IR.html#$s target=_blank>$s</a>"""
  protected def printMem(mem: Sym[_], payload: String*): Unit = {
    val name = mem.name.getOrElse("")
    emit(
      s"""${"  " * ident}<p><div style="padding: 10px; border: 1px;display:inline-block;background-color: #${colorMap(
        mem)}"><font size = "1">${link(s"$mem")} (${mem.memName} "$name")""")
    payload.foreach { p =>
      emit(s"<br>$p")
    }
  ...
  def printControl(lhs: Sym[_], rhs: Op[_]): Unit = {
    ...
    val isFSM = lhs match {
      case Op(_: StateMachine[_]) => " FSM"; case _ => ""
    }
    inCell(src"$lhs", !isLeaf) {
      emit(
        s"""${"  " * ident}<font size = "6">${link(s"${lhs}")}: ${lhs.schedule} $isFSM<font size = "4"> (${lhs.level})</font>""")
      emit(
        s"""${"  " * ident}<br><font size = "2">${lhs.ctx} <font color="grey">- $line</font></font>""")
      ...
      if (lhs.isInnerControl)
        emit(
          s"""${"  " * ident}<p><mark style="border:1px; border-style:solid; border-color:black; padding: 1px; background: #ccc"><font size = "2">Latency=${lat},  ${attentionII}</font></mark></p>""")
      ...
      if (cchain.isDefined)
        emit(
          s"""${"  " * ident}<br><font size = "1">Counter: ${link(cchain.get)}</font>""")
      emit("")
  ...
  def print_stream_info(sym: Sym[_]): Unit = {
    val listens = getReadStreams(sym.toCtrl).map { a =>
      s"$a"
    }
    val pushes = getWriteStreams(sym.toCtrl).map { a =>
      s"$a"
    }
    if (listens.nonEmpty || pushes.nonEmpty) {
      emit(
        s"""${"  " * ident}<div style="border:1px solid black"><font size = "2">Stream Info</font><br><font size = "1"> """)
  ...
  private def sortMems(mems: Seq[Sym[_]]): Seq[Sym[_]] =
    mems.toList
      .map { x =>
        (x, totalVolume(x))
      }
      ...
      .map(_._1)
      .toSeq
  private def singleVolume(x: Sym[_]): Int =
    x.constDims
      .zip(x.getPadding.getOrElse(Seq.fill(x.constDims.length)(0)))
      .map { case (d: Int, p: Int) => d + p }
      .product
  private def totalVolume(x: Sym[_]): Int =
    x.constDims.product * x.instance.depth
  ...
  override def emitFooter(): Unit = {
    ...
    inCell("NBuf Mems", true) {
      ...
      sortMems(nbufs).foreach { mem =>
        ...
        val nBanks =
          if (mem.isLUT | mem.isRegFile) dims else mem.instance.nBanks
        val histR: Map[Int, Int] = mem.readers.toList
          .flatMap { x =>
            x.residualGenerators
          }
          .zip(mem.readers.toList.flatMap { x =>
            if (x.getPorts.isDefined) x.port.broadcast
            else List.fill(x.residualGenerators.size)(0)
          })
          .collect { case (rg, b) if b == 0 => rg }
          .groupBy { lane =>
            lane.zipWithIndex.map { case (r, j) => r.expand(nBanks(j)).size }.product
          }
          .map { case (k, v) => k -> v.size }
        val histW: Map[Int, Int] = mem.writers.toList
          .flatMap { x =>
            x.residualGenerators
          }
          .zip(mem.writers.toList.flatMap { x =>
            if (x.getPorts.isDefined) x.port.broadcast
            else List.fill(x.residualGenerators.size)(0)
          })
          .collect { case (rg, b) if b == 0 => rg }
          ...
          .map { case (k, v) => k -> v.size }
        val allBins =
          (histR.map(_._1) ++ histW.map(_._1)).toList.sorted.distinct
        val hist =
          if (volume > 1)
            (Seq(
              """<div style="display:grid;grid-template-columns: max-content max-content max-content"><div style="border: 1px solid;padding: 5px"><b>muxwidth</b></div> <div style="border: 1px solid;padding: 5px"><b># R lanes</b></div><div style="border: 1px solid;padding: 5px"><b># W Lanes</b></div>""") ++ allBins
              .map { b =>
                s"""<div style="border: 1px solid;padding: 5px">$b</div> <div style="border: 1px solid;padding: 5px">${histR.getOrElse(
                  b,
                  0)}</div><div style="border: 1px solid;padding: 5px">${histW
                  .getOrElse(b, 0)}</div>"""
              } ++ Seq("</div>")).mkString(" ")
          else ""
        printMem(
          ...
          s"lca = ${link(s"$lca")}",
          s"nBufs = $depth",
          s"volume = $volume (dims $dims + pads $pads, bw = $bitwidth, dv = ${mem.getDarkVolume
            .getOrElse(0)})",
          s"nBufs*volume = $bufVolume",
          s"nBanks = $banks, B = $B, a = $alphas, p = $Ps",
          hist
        )
      }
    }
