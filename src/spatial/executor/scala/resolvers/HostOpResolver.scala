...
trait HostOpResolver extends OpResolverBase {
  override def run[U, V](sym: Exp[U, V], execState: ExecutionState): EmulResult = sym match {
    ...
    case Op(amap@ArrayMap(array, applyF, func)) =>
      ...
      val result = Seq.tabulate(arr.size) {
        i =>
          val fpI = SimpleEmulVal(FixedPoint.fromInt(i))
          val applied = runBlock(applyF, Map(applyF.inputA -> arr, applyF.inputB -> fpI), execState)
    ...
    case Op(ArrayZip(arrayA, arrayB, applyA, applyB, func)) =>
      ...
      val size = arrA.size
      val newValues = Seq.tabulate(size) {
        i =>
          val fpI = SimpleEmulVal(FixedPoint.fromInt(i))
          val vA = runBlock(applyA, Map(applyA.inputA -> arrA, applyA.inputB -> fpI), execState)
          val vB = runBlock(applyB, Map(applyB.inputA -> arrB, applyB.inputB -> fpI), execState)
    ...
    case Op(ArrayReduce(array, apply, reduce)) =>
      ...
      val newValues = Seq.tabulate(arr.size) {
        i =>
          val fpI = SimpleEmulVal(FixedPoint.fromInt(i))
    ...
    case Op(SeriesForeach(start, end, step, func)) =>
      ...
      FixedPointRange(rStart, rEnd, rStep, false).foreach {
        i =>
