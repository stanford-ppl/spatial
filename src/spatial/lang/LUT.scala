...
object LUT {
  ...
  /** Allocates a 1-dimensional [[LUT1]] with capacity of `length` and elements of type A.  
    * Elements are read from csv file at compile time with delimiter delim. 
    */
  @api def fromFile[A:Bits](length: Int)(filename: String, delim: String = ","): LUT1[A] = {
    val elems = loadCSVNow[A](filename, delim)(parseValue[A]).map(_.asInstanceOf[Bits[A]])
    stage(LUTNew[A,LUT1](Seq(length),elems))
  }
...
//    stage(LUTNew[A,LUT2](Seq(rows,cols), readData[A](filename)))
//  }
//
//  /** Allocates a 3-dimensional [[LUT3]] with the given dimensions and elements of type A.  
//    * Elements are read from csv file at compile time with delimiter delim. 
...
//    stage(LUTNew[A,LUT3](Seq(d0,d1,d2), readData[A](filename)))
//  }
//
//  /** Allocates a 4-dimensional [[LUT4]] with the given dimensions and elements of type A.  
//    * Elements are read from csv file at compile time with delimiter delim. 
...
//    stage(LUTNew[A,LUT4](Seq(d0,d1,d2,d3), readData[A](filename)))
//  }
//
//  /** Allocates a 5-dimensional [[LUT5]] with the given dimensions and elements of type A.  
//    * Elements are read from csv file at compile time with delimiter delim. 
...
//    stage(LUTNew[A,LUT5](Seq(d0,d1,d2,d3,d4), readData[A](filename)))
//  }
