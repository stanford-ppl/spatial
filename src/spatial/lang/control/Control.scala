...
class Pipe(name: Option[String], ii: Option[Int], directive: Option[UnrollStyle], nobind: Boolean, stopWhen: Option[Reg[Bit]], haltIfStarved: Boolean)
  ...
  @api def apply(func: => Any): Void = unit_pipe(func, stopWhen = stopWhen)
  @rig def apply(ens: Set[Bit], func: => Any): Void = unit_pipe(func, ens, stopWhen = stopWhen)
  @rig def apply(breakWhen: Reg[Bit])(func: => Any): Void = unit_pipe(func, stopWhen = Some(breakWhen))

  def II(ii: Int) = new Pipe(name, Some(ii), directive, nobind, stopWhen, haltIfStarved)
  def POM = new Pipe(name, ii, Some(ParallelOfMetapipes), nobind, stopWhen, haltIfStarved)
  def MOP = new Pipe(name, ii, Some(MetapipeOfParallels), nobind, stopWhen, haltIfStarved)
  def NoBind = new Pipe(name, ii, directive, true, stopWhen, haltIfStarved)
  def haltIfStarved = new Pipe(name, ii, directive, nobind, stopWhen, true)
}
class Stream(name: Option[String], stopWhen: Option[Reg[Bit]], haltIfStarved: Boolean) extends Directives(CtrlOpt(name,Some(Streaming),None,stopWhen,haltIfStarved)) {
  ...
  @api def apply(wild: Wildcard)(func: => Any): Void = this.Foreach(*){_ => func }
