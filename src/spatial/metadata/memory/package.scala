...
package object memory {
  ...
  implicit class MemoryAccessOps(s: Sym[_]) {
    ...
    def addrStream: Sym[_] = s match {
      ...
      case Op(FringeStreamStore(_,_,cmd,_,_)) => cmd //sic
      case Op(FringeDynStore(_,_,cmd,_,_)) => cmd //sic
      //case Op(BVBuild(_,_,cmd,_,_)) => cmd //sic
    ...
    def dataStream: Sym[_] = s match {
      ...
      //case Op(BVBuild(_,_,_,data,_)) => data //sic
    ...
    def ackStream: Sym[_] = s match {
      ...
      case Op(FringeStreamStore(_,_,_,ack,_)) => ack
      case Op(FringeDynStore(_,_,_,ack,_)) => ack
      //case Op(BVBuild(_,_,_,_,ack)) => ack //sic
