...
package object memory {
  ...
  implicit class MemoryOps(mem: Sym[_]) {
    ...
    def optimizedRegType: Option[Accum] = if (!mem.isOptimizedReg) None else 
      mem.writers.collect{ 
      case x if x.op.get.isInstanceOf[RegAccum[_]] => x}.head.asInstanceOf[Sym[_]] match {
        case Op(RegAccumOp(_,_,_,t,_)) => Some(t)
        case Op(_: RegAccumFMA[_]) => Some(Accum.FMA)
        case Op(_: RegAccumLambda[_]) => Some(Accum.Unk)
      }
