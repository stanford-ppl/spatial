  ...
  @rig def transfer[A,Dram[T]<:DRAM[T,Dram],Local[T]<:LocalMem[T,Local]](
    ...
    if (outerLoopCounters.nonEmpty) {
      Stream.Foreach(outerLoopCounters.map{ctr => ctr()}){ is =>
        ...
        val indicesPadded = dram.rawRank.map{i => if (dram.sparseRank.contains(i)) indices(i) else 0.to[I32]}
        val stridesPadded = dram.rawRank.map{i => if (dram.sparseRank.contains(i)) strides(i) else 1.to[I32]}
