...
@op case class SparseTransfer[A,Local[T]<:LocalMem[T,Local]](
  ...
  @rig def lower(old:Sym[Void]): Void = {
    spatialConfig.enablePIR match {
      case true => SparseTransfer.transferPlasticine(old, dram,local,ens,isGather)
      case false => SparseTransfer.transferFPGA(old, dram,local,ens,isGather)
    }
