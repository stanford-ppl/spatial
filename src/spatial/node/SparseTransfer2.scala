...
//@op case class SparseTransfer[A,Local[T]<:LocalMem[T,Local]](
  ...
  //@rig def lower(old:Sym[Void]): Void = SparseTransfer.transfer(old, dram,local,ens,isGather)
...
//object SparseTransfer {
  ...
  //): Void = {
    //if (spatialConfig.enablePIR) {
      //transferPlasticine(old, dram, local, ens, isLoad)
    //} else {
      //transferFPGA(old, dram, local, ens, isLoad)
    //}
  //}

  //@virtualize
  //@rig def transferPlasticine[A,Local[T]<:LocalMem[T,Local]](
    //old:        Sym[Void],
    //dram:    DRAMSparseTile[A],
    //local:   Local[A],
    //ens:     Set[Bit],
  ...
  //): Void = {
    ...
    //val requestLength = dram match {
      //case Op(MemSparseAlias(cond, mem, addr, size, origin)) if mem.size == 1 && size.isEmpty => None
      //case _ => Some(dram.sparseLens().values.head)
    //}
    ...
    //if (isLoad) {
      ...
      //(requestLength) match {
        ////case None if local.isFIFO =>
          ////Stream.Foreach(*, p par p) { (_,i) =>
    ...
    //else {
      ...
      //(requestLength) match {
        //case Some(requestLength) =>
          ...
          //}
        //case None =>
          //error(s"Must specify scatter size for Plasticine ${old.ctx}")
      //}
    //}
  //}

  //@virtualize
  //@rig def transferFPGA[A,Local[T]<:LocalMem[T,Local]](
    //old:        Sym[Void],
    //dram:    DRAMSparseTile[A],
    //local:   Local[A],
    //ens:     Set[Bit],
  ...
  //): Void = {
    ...
    //top.loweredTransfer = isLoad match {
      //case true => SparseLoad
      //case false => SparseStore
    //}
  //}
