...
/** A dynamically-computed store from on-chip to off-chip memory.
  ...
  */
@op case class StreamLoad[A,Dram[T]<:DRAM[T,Dram],Local[T]<:LocalMem[T,Local]](
    dram:     Dram[A],
    params:   Tup2[I32,I32],
    out:      Local[I32],
    p:        scala.Int,
...
object StreamLoad {
  ...
  @rig def transfer[A,Dram[T]<:DRAM[T,Dram],Local[T]<:LocalMem[T,Local]](
    ...
    val top = Stream {
      ...
      val dataBus = StreamIn[I32](GatherDataBus[I32])

      setupBus := (pack(base.unbox.to[I64]*bytesPerWord+dram.address, len.unbox), dram.isAlloc)
