...
trait CounterIterRewriteRule extends RewriteRules {
  ...
  IR.rewrites.add[FixMod[_,_,_]]("UnrollLane", {
    ...
    case (FixMod(iter@PreunrollIter(Final(0), _,Final(1),Final(par)),Final(par2)), _, _) if par == par2 =>
      Some(stage(LaneStatic(iter.asInstanceOf[FixPt[TRUE,_32,_0]], List.tabulate(par) { i => i })))
    // (iter / step) % par if start = 0
    case (FixMod(
      Def(FixDiv(iter@PreunrollIter(Final(0), _,step,Final(par)), step2)),
      Final(par2)
    ), _, _) if par == par2 & step2 == step =>
      Some(stage(LaneStatic(iter.asInstanceOf[FixPt[TRUE,_32,_0]], List.tabulate(par) { i => i })))
    // (iter - start / step) % par
    case (FixMod(
      Def(FixDiv(
        Def(FixSub(iter@PreunrollIter(start, _,step,Final(par)), start2)),
        step2
      )),
      Final(par2)
    ), _, _) if par == par2 & step2 == step & start2 == start =>
      Some(stage(LaneStatic(iter.asInstanceOf[FixPt[TRUE,_32,_0]], List.tabulate(par) { i => i })))
    case _ => None
  })
