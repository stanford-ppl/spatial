...
case class GlobalAllocation(IR: State) extends MutateTransformer {
  ...
  protected def innerBlock[A:Type](lhs: Sym[A], block: Block[_], iters: Seq[I32]): Sym[A] = {
    ...
    val wrMems: Seq[Sym[_]] = wrSyms.values.flatten.toSeq.distinct.filter{x => x.isSRAM || x.isFIFO }
    val rdMems: Seq[Sym[_]] = rdSyms.values.flatten.toSeq.distinct.filter{x => x.isSRAM || x.isFIFO }
