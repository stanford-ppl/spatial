...
//    case op@SplitterEnd(addrs) =>    unrollSplitter(lhs, op)
  ...
  def bankedAccess[A:Bits](
    ...
    case op:SparseSRAMRMW[_,_]    => UVecReadSym(stage(SparseSRAMBankedRMW(mem.asInstanceOf[SparseSRAM1[A]], data, bank, ofs, node.getSparseOp, node.getSparseOrder, barriers, op.key, op.remoteAddr, enss)))
