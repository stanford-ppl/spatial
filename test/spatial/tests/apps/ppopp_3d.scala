...
@spatial class R3D_1 extends SpatialTest {
...
@spatial class R3D_2 extends SpatialTest {
	override def runtimeArgs = "0 3192 0"
	//override val target = Zynq
	// struct size of power of 2
	@struct case class triangle3D(x0 : UInt8, y0 : UInt8, z0 : UInt8, 
...
@spatial class R3D_3 extends SpatialTest {
	override def runtimeArgs = "0 3192 0"
	//override val target = Zynq
	// struct size of power of 2
	@struct case class triangle3D(x0 : UInt8, y0 : UInt8, z0 : UInt8,
	...
	@struct case class triangle2DWithCorners(x0 : UInt8, y0 : UInt8,
								  x1 : UInt8, y1 : UInt8,
								  x2 : UInt8, y2 : UInt8,
								  z  : UInt8, min_x: UInt8, max_x: UInt8, min_y: UInt8, max_y: UInt8, process: Bit)

  @struct case class CandidatePixel(x : UInt8, y : UInt8, z : UInt8, color : UInt8)

	@struct case class Pixel(x : UInt8, y : UInt8, color : UInt8)

	def coloringFB(pixels 			: FIFO[Pixel],
				   frame_buffer		: SRAM2[UInt8],
								 ) : Unit = {
	...
	def rasterization2(
					   sample_triangle2D_with_corners : FIFO[triangle2DWithCorners],
					   fragments 	: FIFO[CandidatePixel]
						) : Unit = {
		...
		val payload = sample_triangle2D_with_corners.deq()
		val process = payload.process
		val sample_triangle2D = triangle2D(payload.x0, payload.y0,
		...
		if ( process ) {
		   	Foreach(payload.min_x.to[I32] until payload.max_x.to[I32] by 1, payload.min_y.to[I32] until payload.max_y.to[I32] by 1){ (x_t, y_t) =>
				val x = x_t.to[Int16]
				val y = y_t.to[Int16]
	...
	/* calculate bounding box for 2D triangle */
	def rasterization1(sample_triangle2D : triangle2D,
										 tri2D_with_corner: FIFO[triangle2DWithCorners]
										) : Unit = {
		...
		val this_check = check_clockwise(sample_triangle2D)

		/* correspomds to the clockwise_vertices function */

		val check_negative = this_check < 0.to[Int]
		val new_x0 = mux(check_negative, sample_triangle2D.x1, sample_triangle2D.x0)
		val new_y0 = mux(check_negative, sample_triangle2D.y1, sample_triangle2D.y0)
		val new_x1 = mux(check_negative, sample_triangle2D.x0, sample_triangle2D.x1)
		val new_y1 = mux(check_negative, sample_triangle2D.y0, sample_triangle2D.y1)

		val new_triangle2D = triangle2D(new_x0, new_y0, new_x1, new_y1,
													 sample_triangle2D.x2, sample_triangle2D.y2,
													 sample_triangle2D.z)

		val min_x =  find_min(new_triangle2D.x0, new_triangle2D.x1, new_triangle2D.x2)
		val max_x =  find_max(new_triangle2D.x0, new_triangle2D.x1, new_triangle2D.x2)
		val min_y =  find_min(new_triangle2D.y0, new_triangle2D.y1, new_triangle2D.y2)
		val max_y =  find_max(new_triangle2D.y0, new_triangle2D.y1, new_triangle2D.y2)

		val new_triangle2D_with_corner = triangle2DWithCorners(new_x0, new_y0, new_x1, new_y1,
													 sample_triangle2D.x2, sample_triangle2D.y2,
													 sample_triangle2D.z, min_x, max_x, min_y, max_y, this_check != 0.to[Int])

		tri2D_with_corner.enq(new_triangle2D_with_corner)

	}

	def projection(sample_triangle3D : triangle3D,
				   proj_triangle2D	 : FIFO[triangle2D],
				   angle			 : Int,
					 i: Int,
					 servicing_id: FIFO[Int]
								) : Unit = {



		val x0 = mux(angle == 0, sample_triangle3D.x0,
					mux(angle == 1, sample_triangle3D.x0, sample_triangle3D.z0))
		val y0 = mux(angle == 0, sample_triangle3D.y0,
					mux(angle == 1, sample_triangle3D.z0, sample_triangle3D.y0))

		val x1 = mux(angle == 0, sample_triangle3D.x1,
					mux(angle == 1, sample_triangle3D.x1, sample_triangle3D.z1))
		val y1 = mux(angle == 0, sample_triangle3D.y1,
					mux(angle == 1, sample_triangle3D.z1, sample_triangle3D.y1))

		val x2 = mux(angle == 0, sample_triangle3D.x2,
		    		 mux(angle == 1, sample_triangle3D.x2, sample_triangle3D.z2))
		val y2 = mux(angle == 0, sample_triangle3D.y2,
					 mux(angle == 1, sample_triangle3D.z2, sample_triangle3D.y2))
		...
		val f3 = sample_triangle3D.x0 / 3.to[UInt8] + sample_triangle3D.x1 / 3.to[UInt8] + sample_triangle3D.x2 / 3.to[UInt8]
		 ...
		 val z =  mux(angle == 0, f1, mux(angle == 1, f2, f3))


		proj_triangle2D.enq(triangle2D(x0, y0, x1, y1, x2, y2, z))
		servicing_id.enq(i)
	}
	...
	def main(args: Array[String]): Void = {
		...
		val vec_sram_len = 64 // num_triangles
		val pxl_tileSize = 32 // 512
		...
		Accel {
			...
			Stream(breakWhen = done).Foreach(*) { i =>


				val triangle3D_vector_sram = FIFO[triangle3D](vec_sram_len)

				'fetch.Pipe {
//					val c = i % vec_sram_len
					if (i == 0) {
//						val load_len = min(vec_sram_len.to[Int], do_triangles - i)
						triangle3D_vector_sram load triangle3D_vector_dram(0 :: do_triangles par 1)
					}
				}

				val tri2D = FIFO[triangle2D](4)
				val tri2D_with_corner = FIFO[triangle2DWithCorners](4)

				val pixels = FIFO[Pixel](pxl_tileSize)

				val flag = FIFO[Bit](4)

				val fragment = FIFO[CandidatePixel](pxl_tileSize)

				val servicing_id = FIFO[Int](8)

				'proj.Pipe { projection(triangle3D_vector_sram.deq(), tri2D, angle, i, servicing_id) }
				'rast1.Pipe { rasterization1(tri2D.deq(), tri2D_with_corner) }
				'rast2.Pipe { rasterization2(tri2D_with_corner, fragment) }
				'zcull.Pipe.II(1) { zculling(fragment, pixels, z_buffer) }
				'color.Pipe { coloringFB(pixels, frame_buffer) }
				'break.Pipe {
					// Need to make sure we can finish even on triangle who did not request updates
					val serviced = servicing_id.deq()
					if (serviced == (do_triangles - 1)) {
						// Spin until drained
						FSM(0)(state => state != 1) { _ => }(state => mux(state == 0 && tri2D.isEmpty && triangle3D_vector_sram.isEmpty && fragment.isEmpty && pixels.isEmpty, 1, 0))
						Pipe{done := true}
					}
				}
			}
			...
			//host_z_buffer store z_buffer
			host_frame_buffer store frame_buffer(0::img_y_size, 0::img_x_size par 16)
		}
