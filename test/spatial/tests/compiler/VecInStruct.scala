...
@spatial class VecInStruct extends SpatialTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    Accel {
      ...
      val tReg = Reg[NestedStruct]
      tReg.dontTouch  // Force placement into a memory
      tReg := y
      val payload: Vec[I32] = tReg.payload
      val tmp = NestedStruct(tReg.index + 1, Vec.fromSeq(Seq(payload(0) + 2, payload(1) + 3)))
      zIdx := tmp.index
