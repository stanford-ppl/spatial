...
@spatial class Convolution_FPGA2 extends SpatialTest { // ReviveMe (LineBuffer)
  val Kh = 3
  val Kw = 3
  val Cmax = 16
    ...
    Accel {
      ...
      Foreach(0 until R) { r =>
        ...
        Foreach(0 until C) { c =>
          ...
          val data = List.tabulate(3){i => List.tabulate(3){j => if (c - 2 + j < 0) 0.to[T] else lb(i,c - 2 + j)}}.flatten
          val kh_data = List.tabulate(3){i => List.tabulate(3){j => kh(i,j)}}.flatten
          val kv_data = List.tabulate(3){i => List.tabulate(3){j => kv(i,j)}}.flatten
          val horz = data.zip(kh_data).map{case (a,b) => a*b}.reduce{_+_}
          val vert = data.zip(kv_data).map{case (a,b) => a*b}.reduce{_+_}

          lineOut(c) = mux( r < 2, 0.to[T], abs(horz) + abs(vert)) // Technically should be sqrt(horz**2 + vert**2)
        }
  ...
  def main(args: Array[String]): Unit = {
    ...
    */
    ...
    println("gold " + gold_sum + " =?= output " + output_sum)
    val cksum = gold_sum == output_sum
    println("PASS: " + cksum + " (Convolution_FPGA2)")
    assert(cksum)
  }
}
