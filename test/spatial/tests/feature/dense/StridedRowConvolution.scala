  ...
  def main(args: Array[String]): Unit = {
    ...
    Accel {
      ...
      Foreach(R/2 by 1){row =>
        ...
        Foreach(C by 1){col =>
          val conv = Reduce(0)(3 by 1, 3 by 1){(r,c) => if (row - 1 + r < 0) 0 else lb(r-1, (col + c)%C)}{_+_} / 9
          line(col) = conv
        }
        out(row,0::C) store line
      }

      // Test lb with "pseudo-transient" load
      val lb2 = LineBuffer.strided[Int](6, C, 2)
      // lb2 load img(0::3, 0::C)
      Foreach(-2 until R/2-2 by 1){row =>
        val line = SRAM[Int](C)
        val rowstart = 4 + row*2
        lb2 load img(rowstart::rowstart+2, 0::C)
        if (row >= 0) {
	        Foreach(C by 1){col =>
	          val conv = Reduce(0)(1 until 6 by 1, 3 by 1){(r,c) => lb2(r, (col + c)%C)}{_+_} / 15
	          line(col) = conv
	        }
