...
@spatial class FloatBasics extends SpatialTest { // Regression (Unit) // Args: 3.2752 -283.70
  ...
  def main(args: Array[String]): Unit = {
    ...
    assert (abs(ffsqrt_result - ffsqrt_gold) < 0.5.to[T], "check Sqrt")
    assert (abs(ffpow_result - ffpow_gold) < 0.01.to[T], "check power of 2")
    assert (abs(ffdiv_result - ffdiv_gold) < 0.01.to[T], "check div")
    assert (abs(ffadd_result - ffadd_gold) < 0.01.to[T], "check add")
    assert (abs(ffmul_result - ffmul_gold) < 0.01.to[T], "check mul")
    assert (abs(ffsub_result - ffsub_gold) < 0.01.to[T], "check sub")
    assert (abs(ffmax_result - ffmax_gold) < 0.01.to[T], "check max")
