...
@spatial class FatConstructor extends SpatialTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val y = ArgOut[Int]
    Accel {
      val many_regs = List.tabulate(256){i => Reg[Int](i)}
      val many_LUTs = List.tabulate(256){i => LUT[Int](3)(0,1,2)}
      Foreach(x by 1){i => 
    ...
    val result = getArg(y)
    val gold = List.tabulate(256){i => i}.reduce{_+_} + ((args(0).to[Int] - 1 )% 3) * 256
