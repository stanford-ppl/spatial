...
@spatial class MuxTest extends SpatialTest {
  override def runtimeArgs: Args = "0 0 1 0 1"

  def main(args: Array[String]): Unit = {
    val sels = List.tabulate(5){i => ArgIn[Int]}
    val ins = List.tabulate(5){i => ArgIn[Int]}
    ...
    val goldhot = List.tabulate(5){i => if (args(i).to[Int] == 0) 0 else (i+1)}.reduce{_|_}
    val goldpri = List.tabulate(5){i => 
